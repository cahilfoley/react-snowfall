...
var Snowflake = /*#__PURE__*/function () {
  function Snowflake(canvas) {
    ...
    this.params = {
      ...
      opacity: _utils.random.apply(void 0, _toConsumableArray(opacity))
    };
  ...
  _createClass(Snowflake, [{
    ...
    value: function getImageOffscreenCanvas(image, size) {
      ...
      if (!(size in sizes)) {
        ...
        canvas.width = size;
        canvas.height = size;
        (_canvas$getContext = canvas.getContext('2d')) === null || _canvas$getContext === void 0 || _canvas$getContext.drawImage(image, 0, 0, size, size);
        sizes[size] = canvas;
      }
    ...
    value: function draw(ctx) {
      if (this.image) {
        ...
        ctx.fillStyle = (0, _utils.hexToRGB)(this.config.color, this.params.opacity);
        ctx.closePath();
        ctx.fill();
      }
    }
